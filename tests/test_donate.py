...
def test_invalid_amount(invalid_amount, user_client):
    response = user_client.post(URL, json={"full_amount": invalid_amount})
    assert response.status_code == 422, (
        f"POST-запрос к {URL} с full_amount == {invalid_amount} должен вернуть"
        "422 статус-код."
    )


def test_create_donation(user_client):
    response = user_client.post(URL, json={"full_amount": 100})
    assert (
        response.status_code == 201
    ), f"POST-запрос к {URL} с full_amount == 100 должен вернуть 201 статус-код."
    data = response.json()
    missing_keys = EXPECTED_KEYS_FOR_USER - data.keys()
    assert not missing_keys, (
        f"При коректном POST-запросе к {URL} в ответе не"
